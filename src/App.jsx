...
function LyricsView({ T, song, elapsed, duration, playing, audioSource, onTogglePlay, onNext, onPrev }) {
  const spotifyHabilitado = audioSource === "spotify";
    ...
    <GlassPanel T={T} className="p-5">
      <div className="rounded-2xl overflow-hidden mb-4 mx-auto" style={{ width: "100%", maxWidth: 220, height: 124, background: "#000" }}>
        {spotifyHabilitado ? (
          <div className="w-full h-full flex flex-col items-center justify-center gap-1" style={{ background: "#0d1f14" }}>
            ...
            <div className="text-[10px] font-bold text-center px-1" style={{ color: "#1DB954" }}>AUDIO EN SPOTIFY</div>
          </div>
        ) : playing && song?.youtubeId ? (
          <iframe
            key={song.youtubeId}
      ...
      {estado === "ok" && (
        <div
          ...
          className="whitespace-pre-line text-center leading-relaxed text-[19px] font-bold"
          style={{ color: T.text, maxHeight: "58vh", minHeight: 260, overflowY: "auto", scrollBehavior: "smooth" }}
        >
          {letra}
        </div>
      )}
...
function SettingsPanel({ T, accentId, setAccentId, mode, setMode, locked, setLocked, password, setPassword, soloMode, setSoloMode, audioSource, setAudioSource, spotifyPlayerStatus, playerSkin, setPlayerSkin, onClose }) {
  return (
    <div className="absolute inset-0 z-30 flex flex-col p-4 overflow-y-auto" style={{ background: `${T.bg}f5`, backdropFilter: "blur(10px)" }}>
      ...
      <GlassPanel T={T} className="p-3.5">
        ...
        <div className="text-[11px] mt-3" style={{ color: T.muted }}>
          ...
          {spotifyPlayerStatus === "idle" && <span>inactivo (activa el modo Spotify)</span>}
        </div>
      </GlassPanel>
    </div>
  );
  ...
  return (
    <div className="min-h-full w-full flex flex-col relative" style={{ background: T.bg, color: T.text }}>
      ...
      <div className="relative flex-1 overflow-y-auto px-4 pt-4">
        ...
        {screenMode === "letras" && (
          <LyricsView
            T={T} song={currentSong} elapsed={elapsed} duration={duration} playing={playing} audioSource={audioSource}
      ...
      {settingsOpen && <SettingsPanel T={T} accentId={accentId} setAccentId={setAccentId} mode={mode} setMode={setMode} locked={locked} setLocked={setLocked} password={password} setPassword={setPassword} soloMode={soloMode} setSoloMode={setSoloMode} audioSource={audioSource} setAudioSource={setAudioSource} spotifyPlayerStatus={spotifyPlayerStatus} playerSkin={playerSkin} setPlayerSkin={setPlayerSkin} onClose={() => setSettingsOpen(false)} />}
    </div>
  );
}
...
export default function App() {
  ...
  // Reproductor web de Spotify: convierte esta misma pestaña en un
  // dispositivo de Spotify, para no depender de abrir la app oficial.
  useEffect(() => {
      ...
      <div style={{ fontFamily: "'Inter', sans-serif" }} className="w-full min-h-screen flex justify-center">
        <div className="w-full max-w-md min-h-screen">
          <VistaInvitado T={T} queue={queue} setQueue={setQueue} onSelectSong={selectSongInRoom} playlists={playlists} setPlaylists={setPlaylists}
            ...
            spotifyPlayerStatus={spotifyPlayerStatus} spotifyDeviceId={spotifyDeviceId} performance={performance} setPerformance={setPerformance}
