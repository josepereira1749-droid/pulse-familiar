...
function RoomPanel({ T, room, onClose }) {
  ...
  const link = "https://pulse-familiar.vercel.app";
  ...
  return (
    <div className="flex flex-col items-end gap-1">
      <div className="text-[9px]" style={{ color: T.muted }}>
        (debug temporal) PIN esperado: "{import.meta.env.VITE_HOST_PIN || "— no está definida —"}"
      </div>
      <div className="flex items-center gap-1.5">
      <input
        type="password"
...
function SettingsPanel({ T, accentId, setAccentId, mode, setMode, locked, setLocked, password, setPassword, soloMode, setSoloMode, audioSource, setAudioSource, spotifyPlayerStatus, playerSkin, setPlayerSkin, hostUnlocked, desbloquearAnfitrion, onClose }) {
  return (
...
function VistaInvitado(props) {
  const { T, queue, setQueue, playlists, setPlaylists, avatar, setAvatar, playing, setPlaying, screenMode, setScreenMode,
    ...
    hostUnlocked, desbloquearAnfitrion,
  ...
  return (
    <div className="min-h-full w-full flex flex-col relative" style={{ background: T.bg, color: T.text }}>
      ...
      {settingsOpen && <SettingsPanel T={T} accentId={accentId} setAccentId={setAccentId} mode={mode} setMode={setMode} locked={locked} setLocked={setLocked} password={password} setPassword={setPassword} soloMode={soloMode} setSoloMode={setSoloMode} audioSource={audioSource} setAudioSource={setAudioSource} spotifyPlayerStatus={spotifyPlayerStatus} playerSkin={playerSkin} setPlayerSkin={setPlayerSkin} hostUnlocked={hostUnlocked} desbloquearAnfitrion={desbloquearAnfitrion} onClose={() => setSettingsOpen(false)} />}
    </div>
  );
}
      ...
      <div style={{ fontFamily: "'Inter', sans-serif" }} className="w-full min-h-screen flex justify-center">
        <div className="w-full max-w-md min-h-screen">
          <VistaInvitado T={T} queue={queue} setQueue={setQueue} onSelectSong={selectSongInRoom} playlists={playlists} setPlaylists={setPlaylists}
            ...
            hostUnlocked={hostUnlocked} desbloquearAnfitrion={desbloquearAnfitrion}
