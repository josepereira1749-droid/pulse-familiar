...
function QueuePanel({ T, queue, open, setOpen, onSelect, onClear }) {
    ...
    <GlassPanel T={T} className="mt-3 overflow-hidden">
      ...
      {open && (
        <div className="px-3 pb-3 space-y-1">
          {queue.map((song) => (
            <button key={song.id} onClick={() => onSelect(song.id)} disabled={!song.youtubeId && !song.spotifyUri} className="w-full flex items-center gap-3 rounded-xl px-2 py-2 text-left" style={{ background: song.playing ? `${T.a1}14` : "transparent", opacity: (song.youtubeId || song.spotifyUri) ? 1 : 0.45 }}>
        ...
        {screenMode === "musica" && (
          currentSong ? (
            <>
              ...
              <QueuePanel T={T} queue={queue} open={queueOpen} setOpen={setQueueOpen} onSelect={selectSong} onClear={() => setQueue([])} />
            </>
          ) : (
