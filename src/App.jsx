...
function SettingsPanel({ T, accentId, setAccentId, mode, setMode, locked, setLocked, password, setPassword, soloMode, setSoloMode, audioSource, setAudioSource, spotifyPlayerStatus, onClose }) {
  return (
...
function VistaInvitado(props) {
  ...
  return (
    <div className="min-h-full w-full flex flex-col relative" style={{ background: T.bg, color: T.text }}>
      ...
      {settingsOpen && <SettingsPanel T={T} accentId={accentId} setAccentId={setAccentId} mode={mode} setMode={setMode} locked={locked} setLocked={setLocked} password={password} setPassword={setPassword} soloMode={soloMode} setSoloMode={setSoloMode} audioSource={audioSource} setAudioSource={setAudioSource} spotifyPlayerStatus={spotifyPlayerStatus} onClose={() => setSettingsOpen(false)} />}
    </div>
  );
}
