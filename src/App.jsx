...
function QueuePanel({ T, queue, open, setOpen, onSelect, onClear, onDelete }) {
    ...
    <GlassPanel T={T} className="mt-3 overflow-hidden">
      ...
      {open && (
        <div className="px-3 pb-3 space-y-1">
          {queue.map((song) => (
            <div key={song.id} className="w-full flex items-center gap-2 rounded-xl px-2 py-2" style={{ background: song.playing ? `${T.a1}14` : "transparent" }}>
...
function VistaInvitado(props) {
  const { T, queue, setQueue, playlists, setPlaylists, avatar, setAvatar, playing, setPlaying, screenMode, setScreenMode,
    ...
    hostUnlocked, desbloquearAnfitrion, bloquearAnfitrion, connectedUsers, deviceSaveError, room,
        ...
        {screenMode === "musica" && (
          currentSong ? (
            <>
              ...
              <QueuePanel T={T} queue={queue} open={queueOpen} setOpen={setQueueOpen} onSelect={selectSong} onClear={() => setQueue([])} onDelete={(id) => setQueue((prev) => prev.filter((s) => s.id !== id))} />
            </>
          ) : (
      ...
      <div style={{ fontFamily: "'Inter', sans-serif" }} className="w-full min-h-screen flex justify-center">
        <div className="w-full max-w-md min-h-screen">
          <VistaInvitado T={T} queue={queue} setQueue={setQueue} onSelectSong={selectSongInRoom} playlists={playlists} setPlaylists={setPlaylists}
            ...
            spotifyPlayerStatus={spotifyPlayerStatus} spotifyDeviceId={salaSpotifyDeviceId} deviceSaveError={deviceSaveError} performance={performance} setPerformance={setPerformance}
