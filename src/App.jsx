...
function nextSongId(queue, currentId, dir) {
  const playable = queue.filter((s) => s.youtubeId || s.spotifyUri);
...
function QueuePanel({ T, queue, open, setOpen, onSelect, onClear, onDelete, currentSongId }) {
    ...
    <GlassPanel T={T} className="mt-3 overflow-hidden">
        ...
        {queue.length > 0 && onClear && (
          confirmando ? (
            <div className="flex items-center gap-1.5 mr-2">
              <button onClick={() => { onClear(); setConfirmando(false); }} className="text-[10px] font-bold px-2 py-1 rounded-full" style={{ background: "#FF3B6B", color: "#fff" }}>Confirmar</button>
              <button onClick={() => setConfirmando(false)} className="text-[10px] font-bold px-2 py-1 rounded-full" style={{ background: T.panel, color: T.muted, border: `1px solid ${T.panelBorder}` }}>Cancelar</button>
            </div>
          ) : (
            <button onClick={() => setConfirmando(true)} className="text-[10px] font-bold px-2 py-1 rounded-full mr-2" style={{ background: T.panel, color: T.muted, border: `1px solid ${T.panelBorder}` }}>
              Vaciar
            </button>
          )
        )}
      ...
      {open && (
        <div className="px-3 pb-3 space-y-1">
          {queue.map((song) => (
            <div key={song.id} className="w-full flex items-center gap-2 rounded-xl px-2 py-2" style={{ background: song.id === currentSongId ? `${T.a1}14` : "transparent" }}>
              <button onClick={() => onSelect(song.id)} disabled={!song.youtubeId && !song.spotifyUri} className="flex-1 flex items-center gap-3 text-left min-w-0" style={{ opacity: (song.youtubeId || song.spotifyUri) ? 1 : 0.45 }}>
                ...
                <span className="text-[10px] font-mono font-bold px-1.5 py-0.5 rounded shrink-0" style={{ color: T.muted, border: `1px solid ${T.panelBorder}` }}>{song.code}</span>
              </button>
              {onDelete && (
                <button onClick={() => onDelete(song.id)} className="shrink-0 p-1.5 rounded-full" style={{ color: T.muted }}>
                  <X size={15} />
                </button>
              )}
            </div>
...
function SettingsPanel({ T, accentId, setAccentId, mode, setMode, locked, setLocked, password, setPassword, soloMode, setSoloMode, audioSource, setAudioSource, spotifyPlayerStatus, playerSkin, setPlayerSkin, hostUnlocked, desbloquearAnfitrion, bloquearAnfitrion, deviceSaveError, onClose }) {
  return (
    <div className="absolute inset-0 z-30 flex flex-col p-4 overflow-y-auto" style={{ background: `${T.bg}f5`, backdropFilter: "blur(10px)" }}>
      ...
      <GlassPanel T={T} className="p-3.5">
        {!hostUnlocked ? (
          <div className="flex items-center justify-between">
            <div className="text-[11px] pr-2" style={{ color: T.muted }}>🔒 Esta sección es solo para el anfitrión de la sala</div>
            <HostPinButton T={T} onUnlock={desbloquearAnfitrion} />
          </div>
        ) : (
          <>
            ...
            <div className="text-[10px] mb-3 p-2 rounded-lg" style={{ background: `${T.a1}15`, color: T.muted }}>
              ⚠️ Mientras seas el anfitrión, mantén esta pestaña abierta y visible en pantalla (no bloquees el celular ni cambies de app) — si no, Spotify puede desconectar el reproductor web. Para fiestas largas, lo mejor es un celular dedicado solo a esto, conectado al cargador.
            </div>
            <a
            ...
            {deviceSaveError && (
              <div className="text-[11px] mt-1.5 p-2 rounded-lg" style={{ background: "#FF3B6B22", color: "#FF3B6B" }}>
                ⚠️ No se pudo guardar el dispositivo en la sala: {deviceSaveError}
              </div>
            )}
            <div className="flex items-center justify-between mt-3 pt-3" style={{ borderTop: `1px solid ${T.panelBorder}` }}>
              <div className="text-[11px]" style={{ color: T.muted }}>✅ Este celular es el anfitrión (crea el dispositivo de la sala)</div>
...
function VistaInvitado(props) {
  const { T, queue, setQueue, playlists, setPlaylists, avatar, setAvatar, playing, setPlaying, screenMode, setScreenMode,
    ...
    hostUnlocked, desbloquearAnfitrion, bloquearAnfitrion, connectedUsers, deviceSaveError, room, currentSongId,
  ...
  const currentSong = queue.find((s) => s.id === currentSongId) || queue[0];
  ...
  const playOnSpotify = async (song) => {
    ...
    try {
      ...
      const roomParam = room?.id ? `&room_id=${encodeURIComponent(room.id)}` : "";
      const url = song.spotifyUri
        ? `/api/spotify-play?uri=${encodeURIComponent(song.spotifyUri)}&name=${encodeURIComponent(song.title)}&artist=${encodeURIComponent(song.artist)}${deviceParam}${roomParam}`
        : `/api/spotify-play?q=${encodeURIComponent(`${song.title} ${song.artist}`)}${deviceParam}${roomParam}`;
  ...
  const [searchTab] = useState("canciones");
  ...
  return (
    <div className="min-h-full w-full flex flex-col relative" style={{ background: T.bg, color: T.text }}>
        ...
        {screenMode === "musica" && (
          currentSong ? (
            <>
              ...
              <QueuePanel T={T} queue={queue} open={queueOpen} setOpen={setQueueOpen} onSelect={selectSong} onClear={() => setQueue([])} onDelete={(id) => setQueue((prev) => prev.filter((s) => s.id !== id))} currentSongId={currentSong?.id} />
            </>
          ) : (
      ...
      {settingsOpen && <SettingsPanel T={T} accentId={accentId} setAccentId={setAccentId} mode={mode} setMode={setMode} locked={locked} setLocked={setLocked} password={password} setPassword={setPassword} soloMode={soloMode} setSoloMode={setSoloMode} audioSource={audioSource} setAudioSource={setAudioSource} spotifyPlayerStatus={spotifyPlayerStatus} playerSkin={playerSkin} setPlayerSkin={setPlayerSkin} hostUnlocked={hostUnlocked} desbloquearAnfitrion={desbloquearAnfitrion} bloquearAnfitrion={bloquearAnfitrion} deviceSaveError={deviceSaveError} onClose={() => setSettingsOpen(false)} />}
    </div>
  );
}
...
export default function App() {
  ...
  const [deviceSaveError, setDeviceSaveError] = useState(null);
  ...
  useEffect(() => {
    ...
    const iniciarPlayer = () => {
      ...
      player.addListener("ready", ({ device_id }) => {
        ...
        if (room?.id) {
          supabase.from("rooms").update({ spotify_device_id: device_id }).eq("id", room.id).then(({ error }) => {
            if (error) {
              console.error("No se pudo guardar spotify_device_id en la sala:", error);
              setDeviceSaveError(error.message || "Error desconocido al guardar");
            } else {
      ...
      player.addListener("not_ready", ({ device_id }) => {
        setSpotifyPlayerStatus("connecting");
        // El navegador suspendió la conexión (pantalla bloqueada, cambio de app, etc.)
        // Reintentamos reconectar automáticamente en vez de quedar mudos.
        setTimeout(() => {
  ...
  // "Latido" de reconexión: cada 20 segundos, reafirmamos la conexión del
  // reproductor web activamente — algunos navegadores móviles desconectan la
  // sesión en silencio (sin avisar con el evento "not_ready"), así que en vez
  // de esperar a que se rompa visiblemente, la reforzamos de forma preventiva.
  useEffect(() => {
    if (!hostUnlocked || audioSource !== "spotify") return;
    const id = setInterval(() => {
    ...
    return () => clearInterval(id);
  }, [hostUnlocked, audioSource]);

  // Mantener la pantalla despierta mientras eres el anfitrión — si la pantalla
  // se bloquea, el navegador puede suspender el reproductor web y desconectarlo.
  useEffect(() => {
    if (!hostUnlocked || audioSource !== "spotify") return;
    let wakeLock = null;
    ...
    const pedirWakeLock = async () => {
      try {
        ...
        /* algunos navegadores no lo soportan, o el usuario cambió de pestaña — no es grave */
      }
    };
  ...
  const [salaCurrentSongId, setSalaCurrentSongId] = useState(null);
  useEffect(() => {
    if (!room?.id) return;
    setSalaSpotifyDeviceId(room.spotify_device_id || null);
    setSalaCurrentSongId(room.current_song_id || null);
    const channel = supabase
      .channel(`room-${room.id}`)
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "rooms", filter: `id=eq.${room.id}` }, (payload) => {
        setSalaSpotifyDeviceId(payload.new.spotify_device_id || null);
        setSalaCurrentSongId(payload.new.current_song_id || null);
      })
      .subscribe();
    return () => { supabase.removeChannel(channel); };
  }, [room?.id]);
  ...
  const setQueue = async (updater) => {
    ...
    // Canciones nuevas (estaban en "next" pero no en "queue"): se insertan
    const added = next.filter((s) => !queue.some((q) => q.id === s.id));
    ...
    // Canciones que ya no están (estaban en "queue" pero no en "next"): se borran
    // Esto cubre tanto borrar una canción puntual como vaciar la cola completa.
    const removed = queue.filter((q) => !next.some((s) => s.id === q.id));
    for (const song of removed) {
      await supabase.from("queue").delete().eq("id", song.id);
    }

    // Actualizamos la vista local de inmediato, sin esperar a que llegue el
    // evento de tiempo real, para que se sienta instantáneo.
    setQueueLocal(next);
  };

  const selectSongInRoom = async (id) => {
    if (!room) return;
    // Una sola actualización atómica en la sala, en vez de dos actualizaciones
    // separadas en la cola — así nunca hay un instante intermedio donde
    // "ninguna" canción parezca estar sonando (eso causaba que la app saltara
    // por error a la primera canción de la lista).
    await supabase.from("rooms").update({ current_song_id: id }).eq("id", room.id);
    setPlaying(true);
  };
      ...
      <div style={{ fontFamily: "'Inter', sans-serif" }} className="w-full min-h-screen flex justify-center">
        <div className="w-full max-w-md min-h-screen">
          <VistaInvitado T={T} queue={queue} setQueue={setQueue} onSelectSong={selectSongInRoom} playlists={playlists} setPlaylists={setPlaylists}
            ...
            spotifyPlayerStatus={spotifyPlayerStatus} spotifyDeviceId={salaSpotifyDeviceId} currentSongId={salaCurrentSongId} deviceSaveError={deviceSaveError} performance={performance} setPerformance={setPerformance}
