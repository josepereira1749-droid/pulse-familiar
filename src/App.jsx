...
const INITIAL_QUEUE = [
  { id: "q0", code: "A2", title: "La Vida Es Un Carnaval", artist: "Celia Cruz", duration: "3:30", votes: 8, addedBy: "José", hue: 0, playing: true },
  { id: "q1", code: "A3", title: "Oye Como Va", artist: "Tito Puente", duration: "4:25", votes: 5, addedBy: "Tía Marcela", hue: 1 },
  { id: "q2", code: "A4", title: "Quimbara", artist: "Celia Cruz", duration: "4:15", votes: 3, addedBy: "Primo Beto", hue: 2 },
  { id: "q3", code: "A5", title: "Rayando el Sol", artist: "Maná", duration: "3:48", votes: 2, addedBy: "Mamá", hue: 3 },
];
...
const NOW_PLAYING = { code: "A2", title: "La Vida Es Un Carnaval", artist: "Celia Cruz", addedBy: "José", duration: 210 };
...
function NowPlayingCard({ T }) {
  return (
    <GlassPanel T={T} className="p-4 flex gap-4 items-start relative overflow-hidden">
      <AlbumArt size={128} colors={[T.a1, T.a2]} />
      <div className="flex-1 min-w-0 pt-1">
        <div className="flex items-start justify-between">
          <div className="text-xs font-bold tracking-wide" style={{ color: T.a1 }}>SONANDO AHORA · {NOW_PLAYING.code}</div>
          <div className="flex items-center gap-2 shrink-0">
            <Heart size={17} style={{ color: T.a1 }} fill={T.a1} />
            <MoreHorizontal size={17} style={{ color: T.muted }} />
          </div>
        </div>
        <div className="text-xl font-black leading-tight mt-1" style={{ color: T.text }}>{NOW_PLAYING.title}</div>
        <div className="text-sm mt-1" style={{ color: T.text }}>{NOW_PLAYING.artist}</div>
        <div className="text-xs" style={{ color: T.muted }}>agregada por {NOW_PLAYING.addedBy}</div>
...
function ProgressControls({ T, playing, onToggle, elapsed }) {
  ...
  return (
    <GlassPanel T={T} className="p-4 mt-3">
      ...
      <div className="flex justify-between text-xs font-semibold mb-3" style={{ color: T.muted }}>
        <span>{fmt(elapsed)}</span><span>{fmt(NOW_PLAYING.duration)}</span>
      </div>
      <div className="flex items-center justify-between px-1">
        <Shuffle size={19} style={{ color: T.muted }} />
        <SkipBack size={22} style={{ color: T.text }} fill={T.text} />
        ...
        <SkipForward size={22} style={{ color: T.text }} fill={T.text} />
...
function QueuePanel({ T, queue, open, setOpen }) {
  return (
    <GlassPanel T={T} className="mt-3 overflow-hidden">
      ...
      {open && (
        <div className="px-3 pb-3 space-y-1">
          {queue.map((song) => (
            <div key={song.id} className="flex items-center gap-3 rounded-xl px-2 py-2" style={{ background: song.playing ? `${T.a1}14` : "transparent" }}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs truncate" style={{ color: T.muted }}>{song.artist} · {song.duration}</div>
              </div>
...
function LyricsView({ T, elapsed }) {
  const active = [...LYRICS_DEMO].reverse().find((l) => elapsed >= l.t) || LYRICS_DEMO[0];
  const idx = LYRICS_DEMO.indexOf(active);
  return (
    <GlassPanel T={T} className="p-5">
      <div className="flex items-center justify-between mb-4">
        <div className="text-xs font-bold tracking-wide" style={{ color: T.a1 }}>LETRA · {NOW_PLAYING.title}</div>
        <span className="text-[9px] font-bold px-2 py-0.5 rounded-full" style={{ background: `${T.a1}22`, color: T.muted }}>DEMO</span>
      </div>
      <div className="space-y-4 py-2">
        {LYRICS_DEMO.map((l, i) => {
          const dist = Math.abs(i - idx);
          return <div key={i} className="text-center transition-all duration-300" style={{ fontSize: i === idx ? 19 : 15, fontWeight: i === idx ? 800 : 600, opacity: i === idx ? 1 : Math.max(0.25, 0.6 - dist * 0.12), color: i === idx ? T.a1 : T.muted }}>{l.line}</div>;
        })}
      </div>
      <div className="text-[10px] text-center mt-4" style={{ color: T.muted }}>Letras reales requieren un proveedor con licencia (Spotify no expone letras por API pública).</div>
    </GlassPanel>
  );
}
  ...
  return (
    <div className="flex items-center justify-around px-2 py-3 border-t" style={{ background: T.navBg, borderColor: T.panelBorder }}>
      {items.map((it) => it.center ? (
        <button key={it.id} className="rounded-full flex items-center justify-center" style={{ width: 44, height: 44, border: `2px solid ${T.a2}`, boxShadow: `0 0 16px -2px ${T.a2}aa` }}><it.icon size={18} style={{ color: T.a2 }} /></button>
      ) : (
...
function SettingsPanel({ T, accentId, setAccentId, mode, setMode, locked, setLocked, password, setPassword, soloMode, setSoloMode, onClose }) {
  return (
...
function VistaInvitado(props) {
  ...
  const [elapsed, setElapsed] = useState(76);
  const nextCode = useLetterCodes(queue.length);

  useEffect(() => { if (!playing) return; const id = setInterval(() => setElapsed((e) => (e + 1) % NOW_PLAYING.duration), 1000); return () => clearInterval(id); }, [playing]);

  const results = q.trim() ? MOCK_RESULTS.filter((r) => (r.title + r.artist).toLowerCase().includes(q.toLowerCase())) : MOCK_RESULTS;
  const addSong = (song) => { setQueue((prev) => [...prev, { id: `q_${Date.now()}`, code: nextCode(), title: song.title, artist: song.artist, duration: "3:20", votes: 1, addedBy: "Tú", hue: song.hue }]); setSearchOpen(false); setQ(""); };
  ...
  return (
    <div className="min-h-full w-full flex flex-col relative" style={{ background: T.bg, color: T.text }}>
      ...
      <div className="relative flex-1 overflow-y-auto px-4 pt-4">
        ...
        {screenMode === "letras" && <LyricsView T={T} elapsed={elapsed} />}
        ...
        {screenMode === "musica" && (
          <>
            <NowPlayingCard T={T} />
            <ProgressControls T={T} playing={playing} onToggle={() => setPlaying((p) => !p)} elapsed={elapsed} />
            <QuickActions T={T} queueCount={queue.length} onOpenQueue={() => setQueueOpen(!queueOpen)} />
            <QueuePanel T={T} queue={queue} open={queueOpen} setOpen={setQueueOpen} />
          </>
        )}
        <div className="h-4" />
      </div>
          ...
          <div className="space-y-2 overflow-y-auto">
            {results.map((r) => (
              <div key={r.id} className="flex items-center gap-3 rounded-2xl px-3 py-2.5 border" style={{ background: T.panel, borderColor: T.panelBorder }}>
                <AlbumArt size={44} colors={HUES[r.hue % HUES.length]} />
                <div className="flex-1 min-w-0"><div className="font-bold text-sm truncate" style={{ color: T.text }}>{r.title}</div><div className="text-xs truncate" style={{ color: T.muted }}>{r.artist}</div></div>
      ...
      {settingsOpen && <SettingsPanel T={T} accentId={accentId} setAccentId={setAccentId} mode={mode} setMode={setMode} locked={locked} setLocked={setLocked} password={password} setPassword={setPassword} soloMode={soloMode} setSoloMode={setSoloMode} onClose={() => setSettingsOpen(false)} />}
    </div>
  );
}
...
export default function App() {
  ...
  const [screenMode, setScreenMode] = useState("concurso");
    ...
    <div className="w-full min-h-screen" style={{ background: "#050609" }}>
      ...
      <div style={{ fontFamily: "'Inter', sans-serif" }} className="w-full min-h-screen flex justify-center">
        <div className="w-full max-w-md min-h-screen">
          <VistaInvitado T={T} queue={queue} setQueue={setQueue} playlists={playlists} setPlaylists={setPlaylists}
            ...
            soloMode={soloMode} setSoloMode={setSoloMode} performance={performance} setPerformance={setPerformance}
            ...
            desordenState={desordenState} setDesordenState={setDesordenState} reign={reign} setReign={setReign} />
        </div>
      </div>
    </div>
  );
