export default async function handler(req, res) {
  ...
  if (!clientId || !clientSecret || !redirectUri) {
    return res.status(500).send(`<html><body style="font-family:sans-serif;padding:24px;">Faltan variables de entorno en Vercel (SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET o SPOTIFY_REDIRECT_URI).</body></html>`);
  }
  ...
  try {
    ...
    return res.status(500).send(`<html><body style="font-family:sans-serif;padding:24px;">Error de red al contactar a Spotify: ${e.message}</body></html>`);
  }
}
