export default async function handler(req, res) {
  try {
    ...
    try {
      ...
      return res.status(502).json({ error: "Spotify no respondió correctamente al pedir el token (puede estar limitando peticiones, intenta de nuevo en unos segundos)" });
    }
    ...
    try {
      ...
      return res.status(502).json({ error: "Spotify no respondió correctamente al buscar el artista (puede estar limitando peticiones, intenta de nuevo en unos segundos)" });
    }
    ...
    while (siguienteUrl && paginas < 25) { // tope de seguridad generoso
      ...
      // Si Spotify nos está limitando (429), a veces responde con texto plano
      // en vez de JSON — hay que detectarlo antes de intentar leerlo como JSON.
      ...
      try {
        ...
        // Respuesta no era JSON válido (texto de error inesperado) — cortamos con lo que ya tenemos
        break;
      }
      ...
      if (siguienteUrl) await new Promise((r) => setTimeout(r, 120));
    }
    ...
    return res.status(200).json({
      ...
      albums: albumes,
      avisoIncompleto: limitadoPorSpotify ? "Spotify limitó las peticiones a mitad de camino — esta lista puede estar incompleta. Intenta buscar de nuevo en unos segundos." : null,
    });
  } catch (e) {
