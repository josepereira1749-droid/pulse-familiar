export default async function handler(req, res) {
  try {
    ...
    function puntuar(track) {
      const nombreCompleto = normalizar(`${track.name} ${track.artists.map((a) => a.name).join(" ")}`);
    ...
    // 2. Ver qué dispositivos de Spotify están disponibles ahora mismo,
    //    para apuntar la reproducción al correcto (evita que suene en otro dispositivo "recordado")
    ...
    const playRes = await fetch(`https://api.spotify.com/v1/me/player/play?device_id=${encodeURIComponent(dispositivoActivo.id)}`, {
      method: "PUT",
    ...
    if (playRes.status === 204) {
      return res.status(200).send(`
        <html><body style="font-family:sans-serif;padding:24px;background:#080912;color:#F4F5FA;">
          ...
          <p style="color:#8B90A8;font-size:12px;margin-top:12px;">Dispositivo: ${dispositivoActivo.name} (${dispositivoActivo.type})</p>
        </body></html>
      `);
    }
