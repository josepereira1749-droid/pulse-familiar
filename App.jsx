...
// ---------- Real YouTube audio player (IFrame Player API) ----------
function useYouTubePlayer({ videoId, onStateChange }) {
...
function NowPlayingCard({ T, song, playerContainerRef }) {
  return (
...
function ProgressControls({ T, playing, onToggle, elapsed, duration }) {
...
function LyricsView({ T, elapsed, song }) {
...
function VistaInvitado(props) {
  ...
  const togglePlay = () => {
    if (playing) yt.pause(); else yt.play();
  };
  ...
  return (
    <div className="min-h-full w-full flex flex-col relative" style={{ background: T.bg, color: T.text }}>
      ...
      <div className="relative flex-1 overflow-y-auto px-4 pt-4">
        ...
        {screenMode === "letras" && <LyricsView T={T} elapsed={elapsed} song={currentSong} />}
        ...
        {screenMode === "musica" && (
          <>
            <NowPlayingCard T={T} song={currentSong} playerContainerRef={yt.containerRef} />
            <ProgressControls T={T} playing={playing} onToggle={togglePlay} elapsed={elapsed} duration={duration} />
